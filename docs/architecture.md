...
`/` es la entrada de descubrimiento del catálogo dentro de la carcasa autenticada. Después de repetir el guard de sesión, renderiza inmediatamente el hero y transmite de forma independiente las primeras seis categorías y los primeros tres productos detrás de límites `Suspense` con `Skeleton`. Cada producto se enriquece en paralelo con su primera imagen y el registro de precio más reciente; los fallos de imagen o precio conservan el producto con esos datos ausentes, mientras que un fallo del listado se aísla dentro de su sección. Las cards destacadas muestran esos datos ya serializables y permiten mantener una selección local de hasta cuatro productos; la selección se descarta al recargar o abandonar la página.

La página y la carga de sus secciones permanecen como Server Components. `FeaturedProductsGrid` es el límite cliente pequeño que recibe sólo los productos enriquecidos y administra la selección; `RemoteProductImage` intenta mostrar la URL remota del catálogo y cambia al asset local cuando la URL falta o la carga falla. La búsqueda navega a `/assistant` mediante `q`; las categorías usan `categoryId` en `/explore` y la comparación usa parámetros `productId` repetidos, en el orden elegido, por ejemplo `/compare?productId=<id-1>&productId=<id-2>`. El parser de `/compare` acepta tanto un valor escalar como repetidos, conserva únicamente UUID válidos, elimina duplicados y limita el resultado a cuatro. La pantalla de comparación consulta esos productos en paralelo y muestra todos los encontrados en el orden seleccionado; si una o más consultas fallan, conserva los resultados disponibles y comunica explícitamente que la comparación es parcial.
