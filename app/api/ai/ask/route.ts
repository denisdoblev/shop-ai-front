...
function errorResponse(statusCode: number): NextResponse<AiAskErrorResponse> {
...
function sanitizeSource(value: unknown): AiAskSource | null {
  ...
  if (
    ...
    !isNullableInteger(pageStart) ||
    !isNullableInteger(pageEnd) ||
...
function sanitizeResponse(value: unknown): AiAskResponse | null {
  if (!isRecord(value) || typeof value.answer !== "string" || !Array.isArray(value.sources)) {
    return null;
  }
  ...
  return { answer: value.answer, sources: sources as AiAskSource[] };
}

async function readRequest(request: Request): Promise<AiAskRequest | null> {
  try {
    ...
    const { productId, question } = body as Record<string, unknown>;
    if (
      typeof productId !== "string" ||
...
export async function POST(request: Request) {
  ...
  try {
    const backendResult = await authenticatedServerRequest<unknown, AiAskRequest>(
      "/api/ai/ask",
      {
        method: "POST",
        body: input,
    ...
    if (error instanceof HttpError && PASSTHROUGH_STATUSES.has(error.status)) {
